...
const Mycart = () => {
  ...
  const deleteItem = (e) => {
    ...
    return fetch(`${URL}/deleteitemfromcart`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    <>
      <header className="header">
        ...
        <NavLink className="logo" to="/">
          {" "}
          <span>Vhecile</span>Hiring{" "}
        </NavLink>

        <nav className="navbar">
          <NavLink to="/">Home</NavLink>
          <NavLink to="/rentbike">Rent vehicles</NavLink>
        </nav>
            ...
            <div className="salecartLidiv" key={items._id}>
              <ul>
                <li style={{ wordSpacing: "10px" }}>
                  Brand: {items.brand} --- Model: {items.model} --- Quantity:{" "}
                  {items.quantity} --- Price: {items.price}Taka{" "}
                  <button id={items._id} onClick={deleteItem} className="btn">
